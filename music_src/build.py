...
def write_manifest():
    ...
    for f in sorted(glob.glob(os.path.join(WORK, '*.json'))):
        ...
        aud = m['id'].startswith('aud')   # audition takes: kept in the build, hidden from the track list
        if m['id'].split('@')[0] not in songs.SONGS and not aud: continue
        e = {k: (round(v, 5) if isinstance(v, float) else v) for k, v in m.items() if k in ('file', 'loop', 'loopStart', 'loopEnd', 'duration', 'title', 'bpm', 'intro')}
        # when the track was last rendered (the Music Room's secret "recently worked on" list sorts by it)
        e['worked'] = int(os.path.getmtime(os.path.join(ROOT, m['file'])))
        if aud: e['hidden'] = True
        man[m['id']] = e
